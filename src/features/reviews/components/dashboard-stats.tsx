...
export function DashboardStats({
  ...
  mostViewed,
  totalLabel = "Críticas",
  editHref = (slug) => `/panel/criticas/${slug}`,
}: DashboardStatsProps) {
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-4">
      <Card data-testid="stat-card" className="gap-1 p-3 sm:p-5">
        <span className="text-xs uppercase leading-tight tracking-wide text-muted-foreground">{totalLabel}</span>
