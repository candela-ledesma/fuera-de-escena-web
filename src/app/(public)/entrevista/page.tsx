...
export default async function InterviewsListPage() {
  const items = await getPublishedReviews("entrevista");
  ...
  return (
    <main>
      <Container className="pt-10 md:pt-16">
        ...
        <div className="mt-10">
          {items.length > 0 ? (
            <ReviewGrid items={items} kind="entrevista" headingLevel="h2" />
          ) : (
            <EmptyState message="Todavía no hay entrevistas publicadas." />
          )}
        </div>
      </Container>
    </main>
